...
namespace PEuler_11
{
    class Program
    {
        ...
        {   // cheating super hard, should be done via file input
            ...
            { 
                for (int j = 0; j<20;j++)
                {
                   //4 ways to check (not 8)-> cardinal: up/down, left/right && Diagonal: UL/DR, UR/DL
                   // Must make sure it is valid to check before going

                    // up/down (checking down)
                    if (i + 3 < 20)
                    {
                        int test = numarray[i][j] * numarray[i + 1][j] * numarray[i + 2][j] * numarray[i + 3][j];

                        if (test > biggestproduct) biggestproduct = test;
                    }

                    // left/right (checking right)
                    if (j + 3 < 20)
                    {
                        int test = numarray[i][j] * numarray[i][j + 1] * numarray[i][j + 2] * numarray[i][j + 3];

                        if (test > biggestproduct) biggestproduct = test;
                    }

                    // UL/DR (Checking Down Right)
                    if(i + 3 < 20)
                    { 
                        if(j + 3 < 20)
                        {
                            int test = numarray[i][j] * numarray[i + 1][j + 1] * numarray[i + 2][j + 2] * numarray[i + 3][j + 3];

                            if (test > biggestproduct) biggestproduct = test;
                        }
                    }

                    // UR/DL (Checking Down Left)
                    if (i + 3 < 20)
                    {
                        if (j - 3 >= 0)
                        {
            ...
            Console.WriteLine("The Largest Product is " + biggestproduct);
            Console.Read();
        }
    }
}
